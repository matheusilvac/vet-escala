...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-3 px-5 py-4 text-left font-display text-base font-bold tracking-tight text-white transition-colors hover:text-white/90 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
